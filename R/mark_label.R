...
place_labels <- function(rects, polygons, bounds, anchors, ghosts) {
    ...
    bbox <- list(x = c(0, bounds[1], bounds[1], 0),
    ...
    if (!is.null(ghosts) && length(ghosts$x) > 0) {
        ...
        ghosts <- Map(function(xmin, xmax, ymin, ymax) {
            list(x = c(xmin, xmax, xmax, xmin), y = c(ymin, ymin, ymax, ymax))
        }, xmin = ghosts$x - 2, xmax = ghosts$x + 2, ymin = ghosts$y - 2, ymax = ghosts$y + 2)
    ...
    for (i in seq_along(rects)) {
        ...
        r <- rects[[i]] / 2 + 2
        rect <- list(x = c(-r[1], r[1], r[1], -r[1]), y = c(-r[2], -r[2], r[2], r[2]))
...
make_label <- function(labels, dims, polygons, ghosts, buffer, con_type, con_border,
    ...
    if (con_type == 'none' || !con_type %in% c('elbow', 'straight')) {
        ...
        connect <- con_fun(labelpos[,1] - labeldims[,1], labelpos[,1] + labeldims[,1],
                           labelpos[,2] - labeldims[,2], labelpos[,2] + labeldims[,2],
                           connect[,1], connect[,2])
        if (con_border == 'one') {
            connect <- with_borderline(labelpos[,1] - labeldims[,1],
...
labelboxGrob <- function(label, x = unit(0.5, "npc"), y = unit(0.5, "npc"),
                      ...
                      default.units = 'mm', hjust = 0, pad = margin(2,2,2,2,'mm'),
                      gp = gpar(), vp = NULL) {
    ...
    if (!is.null(description) && !is.na(description)) {
        ...
        if (is.null(width)) final_width <- min(final_width, as_mm(grobWidth(desc_grob)))
    } else {
        desc_grob <- nullGrob()
    ...
    sep_height <- if (lab_height > 0 && desc_height > 0) as_mm(unit(1, 'lines'), width = FALSE)/2 else as_mm(grobDescent(lab_grob))
    vp <- viewport(
        x = x,
        y = y,
